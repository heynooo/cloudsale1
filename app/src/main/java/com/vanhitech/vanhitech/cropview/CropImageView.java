...
//    private float mScale = 0.0f;//TODO
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (getBitmap() != null) initLayout(mViewWidth, mViewHeight);
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (mIsInitialized) {
            ...
            canvas.drawBitmap(getBitmap(), localMatrix1, mPaintBitmap);
    ...
    private void initLayout(int viewW, int viewH) {
        float imgW = getBitmap().getWidth();
        float imgH = getBitmap().getHeight();
        mImgWidth = imgW;
        mImgHeight = imgH;
        float w = (float) viewW;
        float h = (float) viewH;
    ...
    private Bitmap getBitmap() {//TODO
        Drawable d = getDrawable();
        if (d == null) return null;
        return ((BitmapDrawable) d).getBitmap();//TODO
//        return getRoundImage(((BitmapDrawable) d).getBitmap(),10.f);//TODO
    }
...
////TODO 图片修改成4分之1
//        Bitmap outBitmap = Bitmap.createBitmap(bitWidth/4, bitHight/4,
...
//    }
    ...
    public Bitmap getCroppedBitmap() {
        ...
        Bitmap cropped = Bitmap.createBitmap(getBitmap(), x, y, w, h, null, false);
        //----------2014_4_9---------------TODO
        int bitWidth = cropped.getWidth();
        int bitHight = cropped.getHeight();
        BitmapShader bitmapShader = new BitmapShader(cropped,
                Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
        Paint p = new Paint();
        p.setAntiAlias(true);
        p.setShader(bitmapShader);
        RectF oval3 = new RectF(0, 0, bitWidth, bitHight);// 设置个新的长方形
        Bitmap outBitmap = Bitmap.createBitmap(bitWidth, bitHight,
                Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(outBitmap);
        canvas.drawRoundRect(oval3, 9,9, p);//圆角设置
        canvas.save();
        canvas.restore();
        //-------------------------TODO
//        if (mCropMode != CropMode.CIRCLE) return cropped;
        if (mCropMode != CropMode.CIRCLE) return outBitmap;
        return getCircularBitmap(outBitmap);
    }
    ...
    public Bitmap getRectBitmap() {
        ...
        return Bitmap.createBitmap(getBitmap(), x, y, w, h, null, false);
    }
    ...
    public void setmInitialFrameScale(float mInitialFrameScale) {
        this.mInitialFrameScale = mInitialFrameScale;
    }
